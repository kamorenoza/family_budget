    ...
    >
      <div className="tx-item__row">
        ...
        <div className="tx-item__right tx-item__right--actions">
          ...
          <button type="button" className="tx-item__edit" onClick={() => onEdit(tx)} aria-label="Editar gasto">
...
export function BolsilloAccordion({ tx, used, childExpenses, categoryOf, source, dateLabelOf, onToggle, onEdit, dragMode, dragProps }) {
  ...
  const dragAttr = dragProps
    ? Object.fromEntries(Object.entries(dragProps).filter(([k]) => k !== 'handleProps'))
    : {}
  return (
    <div
    ...
    >
        ...
        <div className="bolsillo__main">
          <div className="bolsillo__line">
            <div className="tx-item__body">
              <div className="tx-item__titlerow">
                <p className="tx-item__title">{tx.description}</p>
                {source?.name && (
                  <span className="tx-item__owner" style={{ background: source.color, color: '#fff' }}>
                    {source.name}
                  </span>
                )}
              </div>
            ...
            <div className="tx-item__right tx-item__right--actions">
              <span className={`tx-item__amount ${available >= 0 ? 'tx-item__amount--income' : 'tx-item__amount--gasto'}`}>
          ...
          <div className="tx-item__bar">
            <div className="tx-item__bar-track">
              <div className="tx-item__bar-fill" style={{ width: `${pct}%`, background: tx.color || 'var(--color-primary)' }} />
            </div>
      ...
      {open && (
        <div className="bolsillo__body">
          ...
          <button type="button" className="bolsillo__edit" onClick={() => onEdit(tx)}>
...
export function IncomeItem({ tx, member, memberColor, memberName, dateLabel, onEdit }) {
  ...
  return (
    <div className="tx-item tx-item--income">
      <div className="tx-item__row">
        {member?.photo ? (
          <img className="tx-item__avatar" src={member.photo} alt={memberName} referrerPolicy="no-referrer" />
        ) : (
        ...
        <div className="tx-item__right tx-item__right--actions">
          <span className="tx-item__amount tx-item__amount--income">{formatCurrency(tx.amount)}</span>
          <button type="button" className="tx-item__edit" onClick={() => onEdit(tx)} aria-label="Editar ingreso">
            <svg viewBox="0 0 24 24" width="20" height="20" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
              <path d="M9 6l6 6-6 6" />
            </svg>
          </button>
